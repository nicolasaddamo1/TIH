...
export class PreloadService implements OnApplicationBootstrap {
  constructor(
    ...
    private readonly productoRepository: Repository<Producto>, // Añadir repositorio de Producto
    @InjectRepository(Factura)
    private readonly facturaRepository: Repository<Factura>,
    @InjectRepository(Cellphone)
    private readonly cellphoneRepository: Repository<Cellphone>, // Añadir repositorio de Cellphone
    @InjectRepository(Cliente)
    private readonly clienteRepository: Repository<Cliente>, // Añadir repositorio de Cliente
    @InjectRepository(Category)
    private readonly categoryRepository: Repository<Category>, // Añadir repositorio de Category
  ) {}

  async onApplicationBootstrap() {
    try {
      await this.preloadCategorias(); // Precargar categorías primero
  ...
  private async preloadCategorias() {
    console.log('Cargando categorías...');
    for (const categoria of preloadData.categorias) {
      try {
        ...
        if (!exists) {
          ...
          console.log(`Categoría ${categoria.nombre} guardada con éxito.`);
        } else {
          console.log(`Categoría ${categoria.nombre} ya existe, omitiendo.`);
        }
      } catch (error) {
        console.error(
          `Error al cargar la categoría ${categoria.nombre}:`,
          error,
        );
      }
    }
    ...
    for (const usuario of preloadData.usuarios) {
      try {
        ...
        if (!exists) {
          ...
          console.log(
            `Usuario ${usuario.nombre} ${usuario.apellido} guardado con éxito.`,
          );
        } else {
          console.log(
            `Usuario con email ${usuario.email} ya existe, omitiendo.`,
          );
        }
      } catch (error) {
  ...
  private async preloadProductos() {
    ...
    for (const producto of preloadData.productos) {
      try {
        // Verificar si el producto ya existe por marca y modelo
        const exists = await this.productoRepository.findOne({
          where: { marca: producto.marca, modelo: producto.modelo },
        });
        ...
        if (!exists) {
          ...
          console.log(
            `Producto ${producto.marca} ${producto.modelo} guardado con éxito.`,
          );
        } else {
          console.log(
            `Producto ${producto.marca} ${producto.modelo} ya existe, omitiendo.`,
          );
        }
      } catch (error) {
        console.error(
          `Error al cargar el producto ${producto.marca} ${producto.modelo}:`,
          error,
        );
      }
    }
    ...
    for (const caja of preloadData.cajas) {
      try {
        ...
        const vendedor = await this.usuarioRepository.findOne({
          where: { id: caja.vendedor },
        });
        const cliente = await this.clienteRepository.findOne({
          where: { id: caja.cliente },
        });
        ...
        if (vendedor && cliente && productos.length > 0) {
          ...
          let comision = null;
          if (caja.comision) {
            comision = caja.comision as Comision; // Asegurándote de que sea del tipo enum esperado
          }
          ...
          const nuevaCaja = this.cajaRepository.create({
            ...
            clienteId: caja.cliente, // Usar el ID del cliente
            vendedorId: caja.vendedor, // Usar el ID del vendedor
          ...
          console.log(
            `Caja para el vendedor ${vendedor.nombre} ${vendedor.apellido} guardada.`,
          );
        } else {
          if (!vendedor) {
            console.log(
              `Vendedor con ID ${caja.vendedor} no encontrado, omitiendo caja.`,
            );
          }
          if (!cliente) {
            console.log(
              `Cliente con ID ${caja.cliente} no encontrado, omitiendo caja.`,
            );
          }
          if (productos.length === 0) {
            console.log(
              `Productos no encontrados para la caja con ID , omitiendo.`,
            );
          }
        }
      } catch (error) {
        console.error(
          `Error al cargar la caja para vendedor ID ${caja.vendedor}:`,
          error,
        );
      }
    }
    ...
    for (const factura of preloadData.facturas) {
      try {
        const usuario = await this.usuarioRepository.findOne({
          where: { id: factura.usuarioId },
        });
        if (usuario) {
          await this.facturaRepository.save(
            this.facturaRepository.create({ ...factura, usuario }),
          );
          console.log(
            `Factura para el usuario ${usuario.nombre} ${usuario.apellido} guardada.`,
          );
        } else {
          console.log(
            `Usuario con ID ${factura.usuarioId} no encontrado, omitiendo factura.`,
          );
        }
      } catch (error) {
        console.error(
          `Error al cargar la factura para usuario ID ${factura.usuarioId}:`,
          error,
        );
      }
    }
  ...
  private async preloadCellphones() {
    ...
    for (const cellphone of preloadData.cellphones) {
      try {
        const exists = await this.cellphoneRepository.findOne({
          where: { marca: cellphone.marca, modelo: cellphone.modelo },
        });
        if (!exists) {
          const nuevoCellphone = this.cellphoneRepository.create({
            ...cellphone,
          ...
          console.log(
            `Celular ${cellphone.marca} ${cellphone.modelo} guardado con éxito.`,
          );
        } else {
          console.log(
            `Celular ${cellphone.marca} ${cellphone.modelo} ya existe, omitiendo.`,
          );
        }
      } catch (error) {
        console.error(
          `Error al cargar el celular ${cellphone.marca} ${cellphone.modelo}:`,
          error,
        );
      }
    }
  ...
  private async preloadClientes(): Promise<void> {
    ...
    for (const cliente of preloadData.clientes) {
      try {
        ...
        console.error(
          `Error al cargar el cliente con DNI ${cliente.dni}:`,
          error,
        );
      }
    }
